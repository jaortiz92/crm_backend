...
| **AC-6** (sin presupuesto, E-6) | Cambiar `budget.status='archived'` ⇒ 200 con todos los `budget/variance/margin_pct_budget = null`, `actual` intacto, `meta.budget_source=null`, warning `"No active non-scenario budget for 2026"`; **nunca** 500. *Errata (2026-09-10, Enmienda A-01 de 02_12): el literal pasó a `"No active budget for {year}"` (y se quitó el predicado `is_scenario=False` de Q0); el texto de arriba se conserva como histórico, sin reescribir.* |
