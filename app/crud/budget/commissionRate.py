"""
CommissionRate CRUD Operations

Master catalog of commission % per product line (Pilar 3 - Commission
Engine).  Validations BR-12 (inverted validity) and BR-13 (active
overlap) are mirrored here as BR-45 governs; SQL CHECK/UNIQUE
constraints are intentionally not used (see spec 02_11 §4.1).
"""
...
from app.models.budget import CommissionRate as CommissionRateModel
from app.schemas.budget import CommissionRateCreate, CommissionRateUpdate
...
def get_commission_rates(
    ...
    """
    query = db.query(CommissionRateModel)
    if id_line is not None:
        query = query.filter(CommissionRateModel.id_line == id_line)
    if active_only:
        query = query.filter(CommissionRateModel.is_active.is_(True))
    if date_ref is not None:
        query = query.filter(
            CommissionRateModel.is_active.is_(True),
            CommissionRateModel.date_from <= date_ref,
            CommissionRateModel.date_to >= date_ref,
        )
    return query.order_by(
        CommissionRateModel.id_commission_rate
    ).offset(skip).limit(limit).all()


def get_commission_rate_by_id(
    db: Session, id_commission_rate: int
) -> Optional[CommissionRateModel]:
    """Get a commission rate by its ID."""
    return db.query(CommissionRateModel).filter(
        CommissionRateModel.id_commission_rate == id_commission_rate
    ).first()
...
def _rate_overlaps(
    ...
    """
    query = db.query(CommissionRateModel).filter(
        CommissionRateModel.date_from <= date_to,
        CommissionRateModel.date_to >= date_from,
        CommissionRateModel.is_active.is_(True),
    )
    if id_line is None:
        query = query.filter(CommissionRateModel.id_line.is_(None))
    else:
        query = query.filter(CommissionRateModel.id_line == id_line)
    if exclude_id is not None:
        query = query.filter(
            CommissionRateModel.id_commission_rate != exclude_id
        )
    return query.first() is not None


def create_commission_rate(
    db: Session, payload: CommissionRateCreate
) -> CommissionRateModel:
    """Create a commission rate (validates BR-12/BR-13: E-CR-1/E-CR-2 -> 400)."""
    ...
    db_rate = CommissionRateModel(**payload.model_dump())
    db.add(db_rate)
    db.commit()
    db.refresh(db_rate)
    return db_rate
...
def update_commission_rate(
    db: Session, id_commission_rate: int, payload: CommissionRateUpdate
) -> Optional[CommissionRateModel]:
    """Update a commission rate with partial merge (only non-None fields).

    Re-runs the same validations on the merged values, excluding the row
    itself from the overlap check (BR-12/BR-13 -> E-CR-1/E-CR-2). Returns
    None if the rate does not exist (404 is resolved by the API layer).
    """
    db_rate = get_commission_rate_by_id(db, id_commission_rate)
    if db_rate is None:
        return None
    ...
    if eff_is_active and _rate_overlaps(
        ...
        exclude_id=db_rate.id_commission_rate,
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
